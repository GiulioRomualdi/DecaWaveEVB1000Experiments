					...
					int *combination)
{
	...
	*best_gdoprate = 1; /* put the worst gdoprate init */
	...
	do {
        ...
        } while (!success && (overlook_count <= 100) && !concentric);
...
//	p1.x = 0.0;			p1.y = 0.0;			p1.z = 1.667;
//	p2.x = 6.268;		p2.y = 0.02;		p2.z = 1.667;
//	p3.x = 3.434;		p3.y = 3.724;		p3.z = 1.667;
//	p4.x = 7.199;		p4.y = 1.588;		p4.z = 2.070; //4th same as 1st - only 3 used for trilateration

//	p1.x = 0.0;			p1.y = 0.0;			p1.z = 0.463;
//	p2.x = 0.0;		    p2.y = 3.348;		p2.z = 0.463;
//	p3.x = 3.845;		p3.y = 1.813;		p3.z = 0.463;
//	p4.x = 2.214;		p4.y = 3.794;		p4.z = 2.905; //4th same as 1st - only 3 used for trilateration

	p1.x = 0.0;			p1.y = 0.0;			p1.z = 0.785;
	p2.x = 0.0;		    p2.y = 2.981;		p2.z = 0.785;
	p3.x = 4.722;		p3.y = 1.721;		p3.z = 0.785;
	p4.x = 0.143;		p4.y = 1.642;		p4.z = 3.009; //4th same as 1st - only 3 used for trilateration

    r1 = (double) range0 / 1000.0;
    r2 = (double) range1 / 1000.0;
    r3 = (double) range2 / 1000.0;

    r4 = (double) range3 / 1000.0;
...
//OLD VERSION int GetLocation(vec3d *best_solution,int use4thAnchor, int range0, int range1, int range2, int range3);
//new version
...
// anchorpos[12] = [p1.x p1.y p1.z p2.x .....]
	...
	p1.x = anchorpos[0];			p1.y = anchorpos[1];			p1.z = anchorpos[2];
	p2.x = anchorpos[3];		    p2.y = anchorpos[4];		p2.z = anchorpos[5];
	p3.x = anchorpos[6];		p3.y = anchorpos[7];		p3.z = anchorpos[8];
	p4.x = anchorpos[9];		p4.y = anchorpos[10];		p4.z = anchorpos[11]; //4th same as 1st - only 3 used for trilateration
    ...
    if (use4thAnchor_for_triangulation) {
        result = deca_3dlocate (&o1, &o2, best_solution, &error, &best_3derror, &best_gdoprate,
                               p1, r1, p2, r2, p3, r3, p4, r4, &combination);
    } else {
        //NOTE: the version used originally did not actually use the 4th anchor pos : 
        //      p4 and r4 where set to p1 and r1 in the call to deca_3dlocate
        result = deca_3dlocate (&o1, &o2, best_solution, &error, &best_3derror, &best_gdoprate,
                               p1, r1, p2, r2, p3, r3, p1, r1, &combination);
    }
